...
<div class="pointer-events-none fixed inset-x-0 top-0 z-50 flex flex-col items-center gap-2 p-4">
  {#each $toasts as toast (toast.id)}
    <div
      class="pointer-events-auto flex w-full max-w-sm items-start gap-3 rounded-lg border p-4 shadow-lg {getStyles(
        toast.type,
      )}"
    ...
    >
      <div
        class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-sm font-bold {getIconStyles(
          toast.type,
        )}"
      >
        {getIcon(toast.type)}
